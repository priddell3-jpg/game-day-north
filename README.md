...
- **`data.json` is served with `max-age=600`.** The page re-reads it on every full pass — startup, the quarter-hour timer and a return to the tab — and asks for a revalidation instead of accepting a copy the browser may hold for ten minutes, otherwise a score the scheduled job has already committed stays invisible. An unchanged file costs a 304 and no body; the API calls keep ordinary caching. On iOS the native bridge does the same by hand: it keeps the ETag beside the saved copy, sends it as `If-None-Match`, and on a 304 neither parses nor rewrites anything. Vercel does return an ETag and a 304 for `/data.json`; checked against the live site on 12 Sep 2026.
...
- **The minute pass asks the score source only.** It tops up whatever schedule is already in hand and reads no file. `data.json` changes every three hours, so it is re-read by the quarter-hour pass, on return to the tab and at startup — not sixty times an hour, which on a phone was a whole download and a rewrite of the offline copy each time.
- **A failing source is asked less, not decided about.** When a score request is unreachable, refused with 429 or answered with a 5xx, the minute poll waits twice as long before the next pass, up to the quarter-hour the slow refresh already keeps, and returns to a minute after the next pass in which every request answers. A 404 is an answer, not an outage. A failed request never changes what is on screen: no score is invented, none is removed, and no game is declared over.
